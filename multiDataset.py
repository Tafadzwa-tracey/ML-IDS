...
# --------------------------
# Title & Research Theme (UNCHANGED)
# --------------------------
...
def get_inference_time(model, X):
    sample = X.iloc[:1000]  # Larger sample = precise measurement for slow models
    start = time.perf_counter()  # Higher precision than time.time()
    model.predict(sample)
    total_time = time.perf_counter() - start
    return total_time / len(sample)  # Seconds per packet
...
metrics = {
    ...
    "MLP": {
        ...
        "inf_time": get_inference_time(mlp_bench, X_test),  # Now accurate for MLP
